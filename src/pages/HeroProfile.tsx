  ...
  return (
    <div className="space-y-6 md:space-y-8 animate-fade-in-up">
      ...
      <div className="grid grid-cols-3 gap-3 md:gap-4">
        <div className="bg-white rounded-2xl p-4 md:p-6 shadow-card border border-water-50/50 animate-fade-in-up card-hover">
          <div className="w-10 h-10 md:w-12 md:h-12 rounded-xl bg-water-50 flex items-center justify-center mb-3">
            <Droplets className="w-5 h-5 md:w-6 md:h-6 text-water-500" />
          </div>
          <div className="font-display text-2xl md:text-3xl gradient-text mb-1">{totalRecords}</div>
          <div className="text-xs md:text-sm text-slate-500">累计换水次数</div>
          <div className="mt-2 text-[10px] md:text-xs text-slate-400">
            共 {totalLiters}L
          </div>
        </div>

        <div className="bg-white rounded-2xl p-4 md:p-6 shadow-card border border-rose-50/50 animate-fade-in-up card-hover" style={{ animationDelay: '0.1s' }}>
          <div className="w-10 h-10 md:w-12 md:h-12 rounded-xl bg-rose-50 flex items-center justify-center mb-3">
            <Heart className="w-5 h-5 md:w-6 md:h-6 text-rose-500 fill-rose-500" />
          </div>
          <div className="font-display text-2xl md:text-3xl text-rose-500 mb-1">{totalLikes}</div>
          <div className="text-xs md:text-sm text-slate-500">总获赞数</div>
          <div className="mt-2 text-[10px] md:text-xs text-slate-400">
            {totalRecords > 0 ? `平均 ${(totalLikes / totalRecords).toFixed(1)} 赞/次` : '暂无数据'}
          </div>
        </div>

        <div className="bg-white rounded-2xl p-4 md:p-6 shadow-card border border-amber-50/50 animate-fade-in-up card-hover" style={{ animationDelay: '0.2s' }}>
          ...
          <div className="text-xs md:text-sm text-slate-500">当前徽章等级</div>
          <div className="mt-2 text-[10px] md:text-xs text-slate-400">
      ...
      <div className="bg-white rounded-3xl p-5 md:p-7 shadow-card border border-purple-50/50 animate-fade-in-up" style={{ animationDelay: '0.25s' }}>
        <div className="flex items-center gap-3 mb-5">
          ...
          <div>
            <h2 className="font-display text-xl md:text-2xl text-slate-800">徽章进阶之路</h2>
            <p className="text-xs md:text-sm text-slate-400">从新手到传奇的成长轨迹</p>
          </div>
        </div>
          ...
          {BADGE_LEVELS.map((badge, idx) => {
            ...
            return (
              ...
              >
                ...
                <div className={`text-[10px] md:text-xs font-semibold whitespace-nowrap ${isCurrent ? badge.color : 'text-slate-500'}`}>
                  {badge.name}
                </div>
                <div className="text-[9px] md:text-[10px] text-slate-400 whitespace-nowrap">
                  {idx === 0 ? '1次起' : `${badge.minRecords}次`}
                </div>
                {isCurrent && (
                  <div className="flex items-center gap-0.5 text-[9px] md:text-[10px] text-purple-500 font-semibold">
        ...
        {badgeProgress.next && (
          <div className="mt-4 bg-gradient-to-r from-purple-50 to-pink-50 rounded-2xl p-4">
            <div className="flex items-center justify-between">
              ...
              <div className="text-right">
                <div className="font-display text-xl text-purple-600">{badgeProgress.progress}%</div>
                <div className="h-2 w-20 bg-slate-100 rounded-full overflow-hidden mt-1">
                  <div
      ...
      <div className="bg-white rounded-3xl p-5 md:p-7 shadow-card border border-water-50/50 animate-fade-in-up" style={{ animationDelay: '0.3s' }}>
        <div className="flex items-center justify-between mb-5">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="font-display text-xl md:text-2xl text-slate-800">换水历史时间线</h2>
              <p className="text-xs md:text-sm text-slate-400">共 {employeeRecords.length} 条换水记录</p>
            </div>
          </div>
        </div>
        ...
        {employeeRecords.length === 0 ? (
          ...
          <div className="relative">
            <div className="absolute left-4 md:left-6 top-2 bottom-2 w-0.5 bg-gradient-to-b from-water-300 via-water-200 to-slate-200" />
              ...
              {timelineGroups.map((group) => (
                <div key={group.date}>
                  <div className="relative pl-12 md:pl-16 mb-3">
                    ...
                    <div className="font-display text-sm md:text-base text-slate-700">{group.label}</div>
                    <div className="text-[10px] md:text-xs text-slate-400">{group.items.length} 次换水</div>
                  </div>
                  ...
                  <div className="space-y-3">
                    {group.items.map((record) => {
                      ...
                      return (
                        ...
                        >
                          <div className="absolute left-3 md:left-5 top-3 w-2.5 h-2.5 rounded-full bg-water-300 ring-4 ring-water-50 z-10" />

                          <div className="bg-gradient-to-r from-water-50/50 to-cyan-50/50 rounded-xl p-3 md:p-4 border border-water-100/50 hover:shadow-md transition-shadow">
                            <div className="flex items-center justify-between">
                              <div className="flex items-center gap-2.5">
                                <span className="text-xl md:text-2xl">{bucket?.icon}</span>
                                <div>
                                  <div className="font-medium text-sm text-slate-700">{bucket?.label}</div>
                                  <div className="text-[10px] md:text-xs text-slate-400">{bucket?.liters}</div>
                                </div>
                              </div>
                              <div className="flex items-center gap-3">
