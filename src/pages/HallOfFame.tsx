  ...
  return (
    <div className="space-y-8 md:space-y-10">
      <div className="animate-fade-in-up">
        ...
        <div className="mb-6 inline-flex p-1 bg-white rounded-xl shadow-card border border-purple-50">
          <button
        ...
        {activeTab === 'heat' && (
          <div className="bg-gradient-to-r from-orange-50 via-amber-50 to-yellow-50 rounded-2xl p-4 md:p-5 border border-orange-100 animate-fade-in-up mb-6">
            <div className="flex items-start gap-3">
              ...
              <div>
                <h3 className="font-display text-base md:text-lg text-slate-800 mb-1">🔥 累计热度分规则</h3>
      ...
      <section className="animate-fade-in-up" style={{ animationDelay: '0.4s' }}>
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="text-sm text-slate-400">
            共 <span className={`font-semibold ${activeTab === 'records' ? 'text-purple-600' : 'text-orange-600'}`}>
        ...
        {activeTab === 'records' ? (
                ...
                >
                  ...
                  <div className="p-5">
                    ...
                    <div className="grid grid-cols-4 gap-2 mb-4">
                      <div className="bg-water-50/80 rounded-xl p-2 text-center">
                        <Droplets className="w-3 h-3 text-water-500 mx-auto mb-0.5" />
                        <div className="font-display text-lg gradient-text">{stat.totalRecords}</div>
                        <div className="text-[9px] text-slate-500">换水</div>
                      </div>
                      <div className="bg-amber-50/80 rounded-xl p-2 text-center">
                        <MessageCircle className="w-3 h-3 text-amber-500 mx-auto mb-0.5" />
                        <div className="font-display text-lg text-amber-500">{stat.totalComments}</div>
                        <div className="text-[9px] text-slate-500">评论</div>
                      </div>
                      <div className="bg-rose-50/80 rounded-xl p-2 text-center">
                        <Heart className="w-3 h-3 text-rose-500 fill-rose-500 mx-auto mb-0.5" />
                        <div className="font-display text-lg text-rose-500">{stat.totalLikes}</div>
                        <div className="text-[9px] text-slate-500">点赞</div>
                      </div>
                      <div className="bg-orange-50/80 rounded-xl p-2 text-center">
                        <Flame className="w-3 h-3 text-orange-500 mx-auto mb-0.5" />
                        <div className="font-display text-lg text-orange-500">{stat.heatScore}</div>
                        <div className="text-[9px] text-slate-500">热度</div>
                      </div>
                    </div>
          ...
          employeesWithHeat.filter(e => e.heatScore > 0).length === 0 ? (
            <div className="bg-white/60 backdrop-blur-sm rounded-3xl p-10 text-center">
              <div className="text-5xl mb-3">🔥</div>
              <h3 className="font-display text-lg text-slate-600 mb-1">暂无热度数据</h3>
              <p className="text-sm text-slate-400">换水、点赞、评论都能累积热度，快来上榜吧！</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-5">
              {employeesWithHeat
                ...
                .map((stat, idx) => {
                  ...
                  return (
                    <Link
                      key={stat.employee.id}
                      to={`/hero/${stat.employee.id}`}
                      className="relative bg-white rounded-3xl shadow-card card-hover overflow-hidden border border-orange-50/50 animate-fade-in-up group block"
                      style={{ animationDelay: `${idx * 0.05}s` }}
                    >
                      {idx < 3 && (
                      ...
                      <div className="p-5">
                        <div className="flex items-start gap-3 mb-4">
                          <div className="relative">
                            <div className="w-16 h-16 md:w-18 md:h-18 rounded-2xl bg-gradient-to-br from-orange-50 to-red-50 flex items-center justify-center text-4xl md:text-5xl border-4 border-white shadow-md group-hover:scale-105 transition-transform">
                              {stat.employee.avatar}
                            </div>
                            <div className="absolute -bottom-1 -right-1 w-8 h-8 rounded-full bg-gradient-to-br from-orange-400 to-red-500 flex items-center justify-center text-lg border-2 border-white shadow-sm">
                              <Flame className="w-4 h-4 text-white" />
                            </div>
                          </div>
                          <div className="flex-1 min-w-0 pt-1">
                            <h3 className="font-display text-xl text-slate-800 truncate mb-1 group-hover:text-orange-600 transition-colors">
                              {stat.employee.name}
                            </h3>
                        ...
                        <div className="grid grid-cols-3 gap-2 mb-4">
                          <div className="bg-water-50/80 rounded-xl p-2 text-center">
                            ...
                            <div className="flex items-center justify-center gap-0.5">
                              ...
                              {stat.recentRecords > 0 && <span className="text-[9px] font-semibold text-orange-500">+{stat.recentRecords}</span>}
                            </div>
                            <div className="text-[9px] text-slate-500">换水</div>
                          </div>
                          <div className="bg-rose-50/80 rounded-xl p-2 text-center">
                            <Heart className="w-3 h-3 text-rose-500 fill-rose-500 mx-auto mb-0.5" />
                            <div className="flex items-center justify-center gap-0.5">
                              <span className="font-display text-base text-rose-500">{stat.likes}</span>
                              {stat.recentLikes > 0 && <span className="text-[9px] font-semibold text-orange-500">+{stat.recentLikes}</span>}
                            </div>
                            <div className="text-[9px] text-slate-500">点赞</div>
                          </div>
                          <div className="bg-amber-50/80 rounded-xl p-2 text-center">
                            <MessageCircle className="w-3 h-3 text-amber-500 mx-auto mb-0.5" />
                            <div className="flex items-center justify-center gap-0.5">
                              <span className="font-display text-base text-amber-500">{stat.comments}</span>
                              {stat.recentComments > 0 && <span className="text-[9px] font-semibold text-orange-500">+{stat.recentComments}</span>}
                            </div>
                            <div className="text-[9px] text-slate-500">评论</div>
                          </div>
                        </div>
                        ...
                        {stat.next ? (
                          <div>
                            <div className="flex items-center justify-between mb-2">
                              <div className="flex items-center gap-1.5">
                                <Target className="w-3.5 h-3.5 text-purple-500" />
                                <span className="text-xs font-medium text-slate-600">称号进度</span>
                              </div>
                              <div className="flex items-center gap-1 text-xs">
                                <span className={stat.current.color}>{stat.current.icon}</span>
                                <ChevronRight className="w-3 h-3 text-slate-400" />
                            ...
                            <div className="flex justify-between mt-1">
                              ...
                              <span className="text-[10px] font-semibold text-purple-500">{stat.progress}%</span>
                            </div>
                          </div>
                        ) : (
                          <div className="bg-gradient-to-r from-orange-50 to-red-50 rounded-xl p-3 text-center">
                            <div className="text-2xl mb-1">🔥</div>
                            <div className="text-xs text-orange-600 font-semibold">最高热度英雄！</div>
                            <div className="text-[11px] text-orange-400">传说级热度贡献者</div>
                          </div>
                        )}

                        <div className="mt-4 flex items-center justify-center gap-1 text-xs text-orange-500 opacity-0 group-hover:opacity-100 transition-opacity">
                          <User className="w-3.5 h-3.5" />
                          <span className="font-medium">查看详情</span>
                        </div>
                      </div>
